...
import "./All.css";
import Modal from "react-modal"; //Popup Model for user
import info from "../../assets/info2.webp"; // image used in popup model
...
const All = () => {
  ...
  //See All Function to visible all the card
  function seeall1() {
        ...
        {fgroup !== "false"
          ? all?.cardslength > 6 && all?.showall === "false"
            ? all.possiblecards.map((card) => (
                <div
                  className="border border-grey-400 drop-shadow-lg relative grid justify-items-center bg-white mt-14">
                  <img
                    src={card.file}
                  ...
                  <p className="text-center line-clamp-2 break-all h-12 w-3/4 text-ellipsis overflow-hidden">
                    {card.gdef}
                  </p>
            ...
            ? all?.possiblecards.map((card) => (
                ...
                >
                  ...
                  <strong className="pt-12 ">{card.gname}</strong>
                  <p
                    className="text-center line-clamp-2 break-all h-12 text-ellipsis overflow-hidden w-3/4"
                  >
                    {card.gdef}
                  </p>
                  <p className="text-center mt-2">
